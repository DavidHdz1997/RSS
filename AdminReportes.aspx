<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AdminReportes.aspx.cs" Inherits="BackRSS.AdminReportes" %>
...
<head runat="server">
    ...
    <!-- Basic -->
		...
		<title>Administracion RSS - Reportes</title>
		<meta name="keywords" content="RSS" />
		<meta name="description" content="Reportes RSS">
...
<%--<head>
	...
	<body>
							...
							<div class="panel-body">
                                ...
                                <div class="form-group">
												...
												<div class="col-md-6">
                                                    ...
                                                    <span class="help-block">Seleccione un proveedor para visualizar sus reportes.</span>
												</div>
                                                <div class="col-md-3">
                                                    <asp:Button ID="btnBuscarProveedor" runat="server" Text="Cargar Reportes" CssClass="btn btn-primary" OnClick="btnBuscarProveedor_Click" />
                                                </div>
											</div>

                                <div class="form-group">
												<label class="col-md-3 control-label">Subir Reporte</label>
												<div class="col-md-6">
													<div class="fileupload fileupload-new" data-provides="fileupload">
														<div class="input-append">
                                                            <asp:FileUpload ID="FileUpload1" runat="server" onchange="fileInfo()" style="display: none;" />
                                                            <asp:TextBox ID="fuReportes" runat="server" CssClass="uneditable-input" ReadOnly="True"></asp:TextBox>
                                                            <asp:LinkButton ID="btnSubirReporte" CssClass="btn btn-primary" runat="server">Seleccionar</asp:LinkButton>
														</div>
                                                ...
                                                <div class="col-md-3">
                                                    <asp:Button ID="btnCargarReporte" runat="server" Text="Subir Reporte" CssClass="btn btn-primary" OnClick="btnCargarReporte_Click" ValidationGroup ="VG3" OnClientClick="if (Page_ClientValidate('VG3') == false) { if (Page_Validators != null) { for (i = 0; i < Page_Validators.length; i++) { Page_Validators[i].className =  'required';  } } } else { this.value='Subiendo...'; this.disabled = true; }" UseSubmitBehavior="false"/>
                                                </div>
											</div>

								<div class="table-responsive">
                                    <asp:GridView ID="dgvAdmReportes" runat="server" AutoGenerateColumns="false" CssClass="table table-bordered table-striped table-condensed mb-none" EmptyDataText = "No hay reportes subidos aun.">
                                        <Columns>
                                            <asp:BoundField DataField="Text" HeaderText="Nombre del Archivo" />
                                            <asp:TemplateField>
                                                <ItemTemplate>
                                                    <asp:LinkButton ID="lnkDownload" Text = "Descargar" CommandArgument = '<%# Eval("Value") %>' runat="server" OnClick = "DownloadFile"></asp:LinkButton>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField>
                                                <ItemTemplate>
                                                    <asp:LinkButton ID = "lnkDelete" Text = "Eliminar" CommandArgument = '<%# Eval("Value") %>' runat = "server" OnClick = "DeleteFile" OnClientClick ="return confirm('¿Estas seguro de eliminar este reporte?')"/>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                        </Columns>
                                    </asp:GridView>
        ...
        <script>
            function fileInfo() {
                document.getElementById('<%=fuReportes.ClientID%>').value = document.getElementById('<%=FileUpload1.ClientID%>').files[0].name;
            }
        </script>
